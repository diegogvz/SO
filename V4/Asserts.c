...
extern int registerA_CPU; // Register A
...
char *elements[]={
	...
	"PCB_SP",	// Process Table item copyOfSPRegister field
...
int Asserts_LoadAsserts() {
	...
	while (fgets(lineRead,MAXLINELENGTH, mf) != NULL && numberAsserts<MAX_ASSERTS) {
		...
		if (time==NULL){
			...
			ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,84,POWERON,lineNumber,ASSERTS_FILE);
			continue;
		}
		...
		if (rc==0){
			...
			ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,82,POWERON,lineNumber,ASSERTS_FILE);
			continue;
		}
		}
		...
		if (rc==0){
				...
				ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,86,POWERON,lineNumber,ASSERTS_FILE, value);
				continue;
		}

		// If memory cell we read address (RMEM_OP, RMEM_O1, RMEM_O2, AMEM_OP, AMEM_O1, AMEM_O2, PCB_ST, PCB_PC, PCB_PR)
		en=elementNumber(a.element);
		if ((en==RMEM_OP) || (en==RMEM_O1) || (en==RMEM_O2) 
			...
			if (rc==0){
				...
				ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,87,POWERON,lineNumber,ASSERTS_FILE);
				continue;    
			}
		}
		else a.address=0;
		...
		if (a.time!=-33)
			...
			Heap_add(numberAsserts,assertsQueue,QUEUE_ASSERTS,&numOfElementsInAssertsQueue);
		else
	...
	if (numberAsserts==MAX_ASSERTS)
			...
			ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,83,POWERON,lineNumber,ASSERTS_FILE);
	else
		asserts[numberAsserts].time=-1; // not necesary...

	ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,82,POWERON,numberAsserts);
...
void assertMsg(int time, char *ele, int expectedValue, int realValue, int addr) {
	...
	ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,88,ERROR, time, ele);
	en=elementNumber(ele);
	...
	if ((en==RMEM_OP) || (en==RMEM_O1) || (en==RMEM_O2) || (en==AMEM_OP) || (en==AMEM_O1) || (en==AMEM_O2) || (en==RMEM) || (en==AMEM)) 
		...
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,91,ERROR, "Memory address",addr);

	if ((en==PCB_ST) || (en==PCB_PC) || (en==PCB_PR)) 
		// printf("; PID: %d", addr);
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,91,ERROR, "PID",addr);

	// printf("\n");
	ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,100, ERROR, "\n");
}
...
void Asserts_CheckAsserts(){
	...
	while (Asserts_IsThereANewAssert(globalCounter)>0) {
		...
		if (asserts[na].time==globalCounter) {
			...
			int en =elementNumber(asserts[na].element);
			if ((en==RMEM_OP) || (en==RMEM_O1) || (en==RMEM_O2)
					|| (en==AMEM_OP) || (en==AMEM_O1) || (en==AMEM_O2) 
					|| (en==RMEM) || (en==AMEM)) {
				ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,96,ERROR,asserts[na].time,asserts[na].element,"Address",asserts[na].address); 
			} 
			else 
				if ((en==PCB_ST) || (en==PCB_PC) || (en==PCB_PR)) {
					ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,96,ERROR,asserts[na].time,asserts[na].element,"PID",asserts[na].address); 
				}
				else
...
void Asserts_CheckOneAssert(int na){
	...
	switch (op) {
		...
		case IR_O2:
				...
				break;
		case PSW:
				if ((registerPSW_CPU!=(unsigned int) asserts[na].value)|| GEN_ASSERTS)
		...
		case PCB_ST:
				...
				if ((valueInPCB != asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,elements[PCB_ST],asserts[na].value,valueInPCB,asserts[na].address);			
				break;
		case PCB_PC:
				valueInPCB=processTable[asserts[na].address].copyOfPCRegister;
				if ((valueInPCB != asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,elements[PCB_PC],asserts[na].value,valueInPCB,asserts[na].address);			
				break;
		case PCB_PR:
				valueInPCB=processTable[asserts[na].address].priority;
				if ((valueInPCB != asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,elements[PCB_PR],asserts[na].value,valueInPCB,asserts[na].address);			
				break;
		case PCB_SP:
				valueInPCB=processTable[asserts[na].address].copyOfSPRegister;
				if ((valueInPCB != asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,elements[PCB_SP],asserts[na].value,valueInPCB,asserts[na].address);			
				break;
		case SP:  
				if ((registerSP_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,elements[SP],asserts[na].value,registerSP_CPU,0);
				break;
		case RA:  
				if ((registerA_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,elements[RA],asserts[na].value,registerA_CPU,0);
				break;
		case RB:  
				if ((registerB_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,elements[RB],asserts[na].value,registerB_CPU,0);
				break;
		case RC:  
				if ((registerC_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,elements[RC],asserts[na].value,registerC_CPU,0);
				break;
#ifdef SLEEPINGQUEUE
		case RD:  
				if ((registerD_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,elements[RD],asserts[na].value,registerD_CPU,0);
				break;
#endif
		default: ;
...
void Asserts_TerminateAssertions(){
	if (numOfElementsInAssertsQueue)
		...
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,92,ERROR,numOfElementsInAssertsQueue);
};