...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	interruptVectorTable[CLOCKINT_BIT]=interruptVectorInitialAddress+NUMBER_OF_CPU_INSTRUCTIONS_PER_INTERRUPT*2;
}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case MEMADD_INST:
			...
			Processor_CheckOverflow(tempAcc, registerMBR_CPU.cell, tempAcc);
			registerPC_CPU++;
			break;
		// Unknown instruction
		default : 
